...
import { calcularIvaInverso } from "@/lib/calculators";
import { IVA_INVERSO_TIPOIVA } from "@/lib/calculators";
import type { IvaInversoResult } from "@/lib/calculators";

export default function IvaInversoForm() {
  const [cantidad, setCantidad] = useState("");
  const [tipoIVA, setTipoIVA] = useState(21);
  const [modo, setModo] = useState<"anadir" | "incluir">("anadir");
  const [resultado, setResultado] = useState<IvaInversoResult | null>(null);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = calcularIvaInverso({ cantidad: cantidadNum, tipoIVA: tipoIVA, modo: modo });
      setResultado(res);
    } catch (e) {
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={isSubmitting} className="btn-primary disabled:opacity-60 disabled:cursor-not-allowed">
            {isSubmitting ? "Calculando..." : "Calcular IVA Inverso"}
          </button>

...
import { calcularPorcentaje } from "@/lib/calculators";
import type { PorcentajeResult } from "@/lib/calculators";

export default function PorcentajeForm() {
  const [valor, setValor] = useState("");
  const [total, setTotal] = useState("");
  const [resultado, setResultado] = useState<PorcentajeResult | null>(null);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = calcularPorcentaje({ valor: valorNum, total: totalNum });
      setResultado(res);
    } catch (e) {
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={isSubmitting} className="btn-primary disabled:opacity-60 disabled:cursor-not-allowed">
            {isSubmitting ? "Calculando..." : "Calcular Porcentaje"}
          </button>

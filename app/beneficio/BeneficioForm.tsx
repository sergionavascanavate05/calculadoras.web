...
import { BENEFICIO_TIPOIVA } from "@/lib/calculators";
...
export default function BeneficioForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = calcularBeneficio({ cantidad: cantidadNum, tipoIVA: tipoIVA, modo: modo });
      setResultado(res);
    } catch (e) {
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={isSubmitting} className="btn-primary disabled:opacity-60 disabled:cursor-not-allowed">
            {isSubmitting ? "Calculando..." : "Calcular Beneficio"}
          </button>

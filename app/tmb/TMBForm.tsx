...
import { calcularTMB } from "@/lib/calculators";
import type { TMBResult } from "@/lib/calculators";
...
export default function TMBForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = calcularTMB({ peso: pesoNum, altura: alturaNum });
      setResultado(res);
    } catch (e) {
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={isSubmitting} className="btn-primary disabled:opacity-60 disabled:cursor-not-allowed">
            {isSubmitting ? "Calculando..." : "Calcular Tasa Metabólica Basal"}
          </button>
      ...
      {resultado && (
        <div className="mt-6 card p-6 animate-slide-up">
          <div className="text-center mb-5">
            <div className="inline-flex items-center justify-center w-20 h-20 rounded-full text-2xl font-bold text-white" style={{ backgroundColor: resultado.color }}>{resultado.tmb}</div>

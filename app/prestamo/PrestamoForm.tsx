...
import { calcularPrestamo } from "@/lib/calculators";
import type { PrestamoResult } from "@/lib/calculators";

export default function PrestamoForm() {
  const [monto, setMonto] = useState("");
  const [interes, setInteres] = useState("");
  const [plazo, setPlazo] = useState("");
  const [resultado, setResultado] = useState<PrestamoResult | null>(null);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = calcularPrestamo({ monto: montoNum, interes: interesNum, plazo: plazoNum });
      setResultado(res);
    } catch (e) {
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={isSubmitting} className="btn-primary disabled:opacity-60 disabled:cursor-not-allowed">
            {isSubmitting ? "Calculando..." : "Calcular Préstamos"}
          </button>

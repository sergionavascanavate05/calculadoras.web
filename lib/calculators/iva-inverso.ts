...
export interface IvaInversoInput {
  cantidad: number;
  tipoIVA: number;
  modo: "anadir" | "incluir";
}

export interface IvaInversoResult {
  sinIVA: number;
  conIVA: number;
  iva: number;
  tipoIVA: number;
...
export const IVA_INVERSO_TIPOIVA = [
  { value: 21, label: "General (21%)" },
  { value: 10, label: "Reducido (10%)" },
  { value: 4, label: "Superreducido (4%)" },
];

export function calcularIvaInverso(input: IvaInversoInput): IvaInversoResult {

...
function clasificarIMC(valor: number): { clasificacion: string; color: string } {
...
export function calcularIMC(input: IMCInput): IMCResult {
  ...
  const { clasificacion, color } = clasificarIMC(redondeado);
  const resultados: CalculatorResult[] = [
    { label: "Tu IMC", value: redondeado.toString() },
  ...
  return { imc: redondeado, clasificacion, color, resultados };
}